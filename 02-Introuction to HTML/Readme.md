...
<font size="1"> 
- Here Input type is text in email, so if we write wrong email like `abcq78` it will not tell whether its right or wrong but if we change input type to email it will show us the error
